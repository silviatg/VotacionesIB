...
var sendVote = function(data) {
  $.ajax({
    ...
    success: function(data) {
      var result = data.trim().split("#"); //Busca en el texto de la página el #, que tendrá: ERROR#textoerror, o OK#
	  
	  //STG. Salía el texto "ERROR" aunque la votación se hiciera bien, y era porque llamaba a envio_interventores.php, y como el interventor
	  // tenia tipo=2 no lo devolvia la select, y daber error en el envio del mail... corregido allí.
	  //Si no vemos por qué falla, podemos mostrar el result[0] para ver que texto de error nos da.
	  //Buscar la cadena: process_result, porque la puede usar en mas páginas, no solo en voto_primarias y envio_interventores.
	  //alert(result[0]);	  
      if (result[0] == 'OK'){
		$("#votacion").hide("slow");		
		//$("#alert-error").find('div').html("");		
		//$("#alert-error").hide(); //STG. No ocultaba el error anterior	
...
$(document).ready(function() {
  ...
  $("#vote").click(function(e) {
	...
	var neutro = showSelectedCandidates('neutro');
    $("#alert-error").hide();
    $("#alert-vot-nul").hide();
    $("#alert-vot-blanc").hide();
    if (men + women + neutro == 0) {
...
//        }
  //    }
//    }
    if ($("#table-men-v").length == 0 || $("#table-women-v").length == 0 || $("#table-neutro-v").length == 0){
  ...
  $("#confirmvote").click(function(e) {
	...
	var neutro = getVotes('neutro');
    var vot_blanc = false;
    var vot_nul = false;    
    if (men.length + women.length + neutro.length == 0) {
...
//    }
    ...
    for (var j = 0, lj = women.length; j < lj; j++) {
 	...
 	for (var j = 0, lj = neutro.length; j < lj; j++) {
      valores += neutro[j][0] + "," + neutro[j][1] + ";";
    }
	...
	var id_votante = $("#id_votante").attr("value"); //STG: Nuevo, para que un admin pueda votar presencialmente por otro votante, quien podrá luego validar su voto con su clave secreta.
