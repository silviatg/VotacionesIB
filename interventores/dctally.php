...
function PollReset()
{
 ...
 global $SavedBal,$NuBal, $Excl, $RankCt, $BalCt, $Remaining, $MultiFlag;
...
function LoadBallots()
{
 ...
 global $FilePath, $Pollname, $RankCt;

 //Read ballot data into array
 $fp=fopen($FilePath.$Pollname."_ballots.txt","r");
 $BalCt=0;
 while(!feof($fp))
 {
...
function XferBallots($who)
{
 ...
 for ($i=0; $i<$Cands; $i++)
 {
  ...
  if ($i!=$who && $ElimFlag[$i]!=2) { $PRecent[$i]=$PLast[$i]; } 
 } 
 $NewTally[$Cands]=$VoteMatrix[$RndNum-1][$Cands];
 $XferPtr=$PFirst[$who];
 if ($ElimFlag[$who]==1 && $PRecent[$who]!=$PLast[$who] && $PRecent[$who]!=$BalCt)
 { $XferAll=false; }
 else
 { $XferAll=true; } 
...
{
 global $Cands,$NewTally,$ElimFlag,$RndNum,$VoteMatrix,$BalCt;
...
for ($i=0; $i<$Cands; $i++)
{
 if (($NewTally[$i]==$TieVotes) && ($Tier!=$i) && ($ElimFlag[$i]==0))
 {
  $NumOfTies++;
  $Ties[$NumOfTies-1]=$i;
 } 
...
while(($TieRnd>0) && ($NumOfTies>0))
{
 ...
 {
  if (($W*$VoteMatrix[$TieRnd][$Ties[$i]]>$W*$TieBreakVotes))
 {
  $TieBreakVotes=$VoteMatrix[$TieRnd][$Ties[$i]];
  $TieBreaker=$i;
 } 
} 

$TieTemp=0;
$OldTieBreaker=$Ties[$TieBreaker];
for ($i=0; $i<=$NumOfTies; $i++)
{
 if (($VoteMatrix[$TieRnd][$Ties[$i]]==$TieBreakVotes) && ($i!=$TieBreaker))
 {
  $TieTemp++;
  $Ties[$TieTemp-1]=$Ties[$i];
 } 
} 

$Ties[$TieTemp]=$OldTieBreaker;
$NumOfTies=$TieTemp;
} 
...
for ($i=0; $i<$Cands; $i++)
{
 ...
 if (($NewTally[$i]>$WinnerVotes) && (($ElimFlag[$i]==0) || ($ElimFlag[$i]==2)))
 {
  $WinnerVotes=$NewTally[$i];
  $Winner=$i;
 } 

 if ((round($NewTally[$i],10)>round($Thresh,10)) && ($ElimFlag[$i]==0))
 {
  $ElimFlag[$i]=2;
  $Elected++;
  $WonInRound[$i]=$RndNum;
...
if (round($WinnerVotes,10)>round($Thresh,10))
{
 ...
 {
  $Surplus[$Winner]=($WinnerVotes-$Thresh)/$XferMatrix[$WonInRound[$Winner]][$Winner];
 }
 $ElimFlag[$Winner]=1;
 $ElimVal=($Winner+1);
