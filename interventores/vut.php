...
include('seguri_inter.php'); ?>
   ...
   <?php 
   $_SESSION['recarga']=""; //limpiamos la sesion de recarga que impide en la siguiente pagina hacer f5 e incluir varias veces los mismos datos 	
...
$conta = "SELECT id,nombre_usuario, id_vut,imagen_pequena FROM $tbn7 WHERE id_votacion = '$idvot' ORDER BY ID ";
...
<?php for ($i1=1; $i1<=$ListLength; $i1++)
  {
   ...
   if ($Suffixnum==1) { echo " o"; }
   if ($Suffixnum==2) { echo " do"; }
   if ($Suffixnum==3) { echo " o"; }
...
<h3><span class="label label-warning">Los datos incluidos no podran ser borrados, tenga cuidado de no cometer errores</span></h3>

<input type="submit" name="submit" id="castvote"  class="btn btn-lg btn-primary pull-right"
value="Incluir voto" onmouseover="vote_check();" onfocus="vote_check();">
